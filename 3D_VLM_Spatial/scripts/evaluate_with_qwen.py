#!/usr/bin/env python3
"""Use Qwen (DashScope OpenAI-compatible API) to judge predictions vs. ground-truth answers."""
...
try:
    ...
    raise SystemExit("Install openai to use this script.") from exc
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Qwen-based evaluation of QA predictions")
    parser.add_argument("--predictions", type=Path, required=True, help="Predictions JSONL (with question/answer/prediction)")
    parser.add_argument("--output", type=Path, required=True, help="Where to write Qwen judgments JSON")
    parser.add_argument("--model", default="qwen-plus", help="Qwen model name")
    parser.add_argument("--batch-size", type=int, default=5, help="Number of QA pairs per call")
...
def _generate_with_retry(client: OpenAI, model: str, prompt: str, temperature: float, max_tokens: int,
                         max_retries: int, initial_backoff: float, max_backoff: float, backoff_mult: float):
    attempt = 0
    ...
    while True:
        try:
            return client.chat.completions.create(
                model=model,
                messages=[
                    {"role": "system", "content": "You are a strict medical QA evaluator."},
...
def judge_with_model(
    ...
    for idx in tqdm(range(0, len(preds), batch_size), desc=f"{model_name} evaluating", unit="batch"):
        ...
        try:
            ...
            print(f"[WARN] Qwen call failed: {exc}")
            for record in batch:
                item = {
        ...
        try:
            ...
            print(f"[WARN] Failed to parse Qwen output for {model_name}:\n", text)
            for record in batch:
                item = {
                    "case_id": record.get("case_id"),
                    "question": record.get("question"),
...
def main() -> None:
    ...
    if not api_key:
        raise SystemExit("Set DASHSCOPE_API_KEY before running.")
