#!/usr/bin/env python3
"""Use GPT (OpenAI API) to judge predictions vs. ground-truth answers."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="GPT-based evaluation of QA predictions")
    parser.add_argument("--predictions", type=Path, required=True, help="Predictions JSONL (with question/answer/prediction)")
    parser.add_argument("--output", type=Path, required=True, help="Where to write GPT judgments JSON")
...
def judge_with_model(
    ...
    for idx in tqdm(range(0, len(preds), batch_size), desc=f"{model_name} evaluating", unit="batch"):
        ...
        try:
            ...
            print(f"[WARN] GPT call failed: {exc}")
            for record in batch:
                item = {
        ...
        try:
            ...
            print(f"[WARN] Failed to parse GPT output for {model_name}:\n", text)
            for record in batch:
                item = {
                    "case_id": record.get("case_id"),
                    "question": record.get("question"),
