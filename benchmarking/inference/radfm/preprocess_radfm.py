#!/usr/bin/env python3
"""
Preprocess CT volumes for RadFM.

DeepTumorVQA Table 2: direct resize to 256x256x64.
RadFM expects image tensor shaped [C, H, W, D], commonly C=3.
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Preprocess CT volumes for RadFM")
    p.add_argument("--input-jsonl", type=Path, required=True, help="JSONL with image_path/question[/answer]")
    p.add_argument("--nifti-root", type=Path, required=True, help="Root of original NIfTI files (valid_fixed)")
    p.add_argument("--output-root", type=Path, required=True, help="Where to write .npy volumes")
    p.add_argument("--output-jsonl", type=Path, required=True, help="Output JSONL with updated image_path")
    p.add_argument("--depth", type=int, default=64, help="Target depth (D)")
    ...
    p.add_argument("--clip-min", type=float, default=None, help="Optional intensity min clip")
    p.add_argument("--clip-max", type=float, default=None, help="Optional intensity max clip")
    return p.parse_args()
...
def main() -> None:
    ...
    updated = []
    for rec in tqdm(list(iter_jsonl(args.input_jsonl)), desc="Preprocessing for RadFM"):
        ...
        if not out_path.is_file():
            ...
            if args.clip_min is not None or args.clip_max is not None:
                lo = args.clip_min if args.clip_min is not None else arr.min()
                hi = args.clip_max if args.clip_max is not None else arr.max()
                arr = np.clip(arr, lo, hi)
            arr = normalize_minmax(arr)
            arr = np.transpose(arr, (1, 2, 0))  # H,W,D
            arr = np.stack([arr, arr, arr], axis=0)  # 3,H,W,D
